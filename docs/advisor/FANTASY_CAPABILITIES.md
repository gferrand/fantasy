...
| `/watch outlook` | watchlist stats → `run_watchlist_web_briefing` | web-backed outlook | none | Yes: facts plus public research. |
| `/watch recommend` | recommendation context + swap signals | web-backed manual swap recommendation | none | Yes. |
| `/injury opportunities` | `load_injury_opportunities_context` | web-enriched deterministic injury inventory | none | Yes; research stays separate. |
| `/trade propose` | persisted fixture schedule + `load_trade_proposal_context` | web-backed manual trade recommendation | none | Yes: legal context/packages. |
...
| `/gameweek prepare` | `load_gameweek_prepare_context` | web-backed preparation report | none | Yes; matchup unavailable. |
...
Specialist commands
  ...
  (waiver path can instead route semantically to web/Codex)
```
