...
- Require valid JSON with `schema_version=1`, `complete=true`, a recent `retrieved_at`, the expected league ID, and the fields `league`, `state`, `users`, `rosters`, `players`, `stats`, `transactions`, `completed_trades_today`, `available_players`, and `team_swap_recommendations`.
- Use this feed for the live league, scoring settings, roster, player metadata, stats, current-round transactions, completed trades, the bounded pickup shortlist, and the manual-review swap considerations.
...
- Treat `team_swap_recommendations` as current-season, same-position scoring signals only. Before recommending a swap, verify the target's role, availability, injury status, fixture context, and that Sleeper shows an Add option. Never imply that a transaction occurred.
