...
class DiscordBotTests(unittest.TestCase):
    ...
    def test_rotation_renders_moneyball_board_and_requires_scannable_verdicts(self):
        source = Path(discord_bot.__file__).read_text(encoding="utf-8")
        rotation_source = source.split("async def rotation_command", 1)[1][:7_000]
...
class DiscordGameweekPresentationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_prepare_uses_the_contract_and_suppresses_link_previews(self):
        ...
        context = type("Context", (), {"payload": {"gameweek": 4, "forecast": {"available": True, "total_display": "Projected XI: 4.0 Kick & Run pts", "required_fragments": ("Projected XI: 4.0 Kick & Run pts", forecast_line), "projected_xi_lines": ("**F — Player One** · est. 4.0 Kick & Run pts",)}, "your_team": {"players": [{"player_id": "p1", "forecast": {"display": forecast_line}}]}}, "retrieved_at": "now"})()
        ...
        self.assertEqual(finalizer.await_args.kwargs["required_analysis_fragments"], ("Projected XI: 4.0 Kick & Run pts",))
