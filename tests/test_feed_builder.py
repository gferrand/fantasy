...
class FeedBuilderTests(unittest.TestCase):
    def test_core_feed_keeps_scoring_aware_pickup_shortlist(self):
        snapshot = {
            ...
            "state": {},
            "users": [{"user_id": "1127171221277331456", "display_name": "Manager", "metadata": {}}],
            "rosters": [{"roster_id": 1, "owner_id": "1127171221277331456", "players": ["owned"]}],
            "players": {
            ...
            "stats": [
                {"player_id": "owned", "stats": {"pos_f_g": 0, "gp": 1, "gs": 1, "min": 90}, "player": {}},
                {"player_id": "free", "stats": {"pos_f_g": 1, "gp": 1, "gs": 1, "min": 90}, "player": {}},
            ],
            "transactions": [],
        }
        feed = build_feed(snapshot)
        self.assertFalse(feed["available_players_complete"])
        self.assertEqual(feed["available_players_limit"], 30)
        ...
        recommendation = feed["team_swap_recommendations"][0]
        self.assertEqual(recommendation["add"]["name"], "Available")
        self.assertEqual(recommendation["drop"]["name"], "Owned")
        self.assertEqual(recommendation["position"], "F")
        self.assertEqual(recommendation["current_season_point_gain"], 9.0)
